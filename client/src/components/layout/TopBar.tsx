...
export function TopBar() {
  ...
  return (
    <header className="glass sticky top-3 z-30 mx-auto mt-4 flex w-[95%] max-w-7xl items-center justify-between gap-4 rounded-2xl px-4 py-3 md:px-6">
      ...
      <nav className="hidden gap-2 rounded-xl border border-slate-700/60 bg-slate-950/40 p-1.5 text-sm text-slate-300 lg:flex">
        {nav.map((item) => (
          <Link
            key={item.href}
            href={item.href}
